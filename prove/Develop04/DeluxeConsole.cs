public static class DeluxeConsole
{
    ...
    /// with longer pauses applied to punctuation, then writes a newline at the end.
    /// </summary>
    /// <param name="inputString">
    /// The text to be written to the console. If empty or omitted, only a newline is written.
    /// </param>
    ...
    // i spent so long trying to get the XML description to work and it's still half baked :/
    public static void WriteLineDeluxe(string inputString="", int inputSpeed=3)
    {
        if (inputSpeed > 4)
        {
            inputSpeed = 4;
